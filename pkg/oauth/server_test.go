	...
	for _, tt := range []struct {
		...
		},
		{
			...
			StoreCheck: func(t *testing.T, s *mockStore) {
				...
				a.So(s.req.authorization.ClientIds, should.Resemble, mockClient.Ids)
				a.So(s.calls, should.Contain, "CreateAuthorizationCode")
				a.So(s.req.authorizationCode.UserIds, should.Resemble, *mockUser.GetIds())
				a.So(s.req.authorizationCode.ClientIds, should.Resemble, mockClient.Ids)
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			StoreCheck: func(t *testing.T, s *mockStore) {
				...
				a.So(s.req.token.ClientIds, should.Resemble, mockClient.Ids)
