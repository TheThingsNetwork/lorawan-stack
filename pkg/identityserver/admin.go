...
func (s *adminService) GetUser(ctx context.Context, req *ttnpb.UserIdentifier) (*ttnpb.User, error) {
	...
	found, err := s.store.Users.GetByID(req.UserID, s.config.Specializers.User)
	if err != nil {
		return nil, err
	}
...
func (s *adminService) UpdateUser(ctx context.Context, req *ttnpb.UpdateUserRequest) (*pbtypes.Empty, error) {
	...
	err = s.store.Transact(func(tx *store.Store) error {
		found, err := tx.Users.GetByID(req.User.UserID, s.config.Specializers.User)
		if err != nil {
			return err
		}
		user := found.GetUser()
...
func (s *adminService) ResetUserPassword(ctx context.Context, req *ttnpb.UserIdentifier) (*ttnpb.ResetUserPasswordResponse, error) {
	...
	err = s.store.Transact(func(tx *store.Store) error {
		found, err := tx.Users.GetByID(req.UserID, s.config.Specializers.User)
		if err != nil {
			return err
		}
...
func (s *adminService) DeleteClient(ctx context.Context, req *ttnpb.ClientIdentifier) (*pbtypes.Empty, error) {
	...
	err = s.store.Transact(func(tx *store.Store) error {
		...
		user, err := tx.Users.GetByID(found.GetClient().Creator.UserID, s.config.Specializers.User)
		if err != nil {
			return err
		}
