...
func TestInvitationStore(t *testing.T) {
	...
	WithDB(t, func(t *testing.T, db *gorm.DB) {
		...
		newUser, err := GetUserStore(db).CreateUser(ctx, &ttnpb.User{
			Ids:                 &ttnpb.UserIdentifiers{UserId: "new-user"},
			PrimaryEmailAddress: "new-user@example.com",
		})
		if err != nil {
			panic(err)
		}

		err = store.SetInvitationAcceptedBy(ctx, "wrong-invitation-token", newUser.GetIds())

		if a.So(err, should.NotBeNil) {
			a.So(errors.IsNotFound(err), should.BeTrue)
		}

		err = store.SetInvitationAcceptedBy(ctx, "invitation-token", newUser.GetIds())
