...
func (is *IdentityServer) createApplication(ctx context.Context, req *ttnpb.CreateApplicationRequest) (app *ttnpb.Application, err error) {
	if err = blacklist.Check(ctx, req.Ids.GetApplicationId()); err != nil {
		return nil, err
	}
	...
	err = is.withDatabase(ctx, func(db *gorm.DB) (err error) {
		...
		if len(req.ContactInfo) > 0 {
			...
			app.ContactInfo, err = store.GetContactInfoStore(db).SetContactInfo(ctx, app.Ids, req.ContactInfo)
			if err != nil {
				return err
			}
		}
	...
	events.Publish(evtCreateApplication.NewWithIdentifiersAndData(ctx, &req.Ids, nil))
	return app, nil
}
...
func (is *IdentityServer) getApplication(ctx context.Context, req *ttnpb.GetApplicationRequest) (app *ttnpb.Application, err error) {
	...
	if err = rights.RequireApplication(ctx, req.ApplicationIds, ttnpb.RIGHT_APPLICATION_INFO); err != nil {
	...
	err = is.withDatabase(ctx, func(db *gorm.DB) (err error) {
		app, err = store.GetApplicationStore(db).GetApplication(ctx, &req.ApplicationIds, req.FieldMask)
		if err != nil {
			return err
		}
		if ttnpb.HasAnyField(req.FieldMask.GetPaths(), "contact_info") {
			app.ContactInfo, err = store.GetContactInfoStore(db).GetContactInfo(ctx, app.Ids)
			if err != nil {
				return err
			}
		}
...
func (is *IdentityServer) listApplications(ctx context.Context, req *ttnpb.ListApplicationsRequest) (apps *ttnpb.Applications, err error) {
	...
	for i, app := range apps.Applications {
		if rights.RequireApplication(ctx, app.Ids, ttnpb.RIGHT_APPLICATION_INFO) != nil {
...
func (is *IdentityServer) updateApplication(ctx context.Context, req *ttnpb.UpdateApplicationRequest) (app *ttnpb.Application, err error) {
	if err = rights.RequireApplication(ctx, req.Ids, ttnpb.RIGHT_APPLICATION_SETTINGS_BASIC); err != nil {
		return nil, err
	}
	...
	events.Publish(evtUpdateApplication.NewWithIdentifiersAndData(ctx, &req.Ids, req.FieldMask.GetPaths()))
	return app, nil
}
