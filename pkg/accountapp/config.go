...
// UIConfig is the combined configuration for the Account app UI.
...
// FrontendConfig is the configuration for the Account app frontend.
type FrontendConfig struct {
	Language    string `json:"language" name:"-"`
	StackConfig `json:"stack_config" name:",squash"`
}

// Config is the configuration for the Account app server.
type Config struct {
	Mount       string   `name:"mount" description:"Path on the server where the account app (and its OAuth services) will be served"`
	UI          UIConfig `name:"ui"`
