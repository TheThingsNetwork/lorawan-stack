...
var (
	...
	errDeviceRegistryInitialize  = errors.DefineInternal("device_registry_initialization_failed", "Device Registry initialization failed")
	errDeviceStoreFailed         = errors.DefineInternal("device_store_failed", "failed to store device")
	...
	errInvalidRX2DataRateIndex   = errors.DefineInvalidArgument("invalid_rx2_data_rate_index", "invalid RX2 data rate index")
	errLoRaAndFSK                = errors.DefineInvalidArgument("lora_and_fsk", "both LoRa and FSK modulation is specified")
	errMACEncodeFailed           = errors.DefineInternal("mac_encode_failed", "failed to encode MAC commands")
	errMACEncryptFailed          = errors.DefineInternal("mac_encrypt_failed", "failed to encrypt MAC commands")
	...
	errScheduleFailed            = errors.Define("schedule_failed", "all Gateway Servers failed to schedule the downlink")
	errJoinFailed                = errors.Define("join_failed", "all Join Servers failed to handle join")
