...
const getGatewayBookmarkNameLogic = createRequestLogic({
  type: gateways.GET_GTW_BOOKMARK_NAME,
  process: async ({ action }) => {
    const { payload } = action
    const { id = {} } = payload
    const gtw = await tts.Gateways.getById(id, 'name')
    trackEntityAccess(GATEWAY, id)
