...
func (j *GoGoJSONPb) marshalTo(w io.Writer, v interface{}) error {
...
func unmarshalGoGoJSONPb(data []byte, v interface{}) error {
	...
	return decodeGoGoJSONPb(d, v)
}

func decodeGoGoJSONPb(d *json.Decoder, v interface{}) error {
	p, ok := v.(proto.Message)
	if !ok {
...
func decodeNonProtoField(d *json.Decoder, v interface{}) error {
	...
	if rv.Kind() == reflect.Map {
		...
		for k, v := range m {
			...
			if err := unmarshalGoGoJSONPb([]byte(*v), bv.Interface()); err != nil {
				return err
			}
