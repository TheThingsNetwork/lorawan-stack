...
// ValidateFields checks the field values on AssertGatewayRightsResponse with
// the rules defined in the proto definition for this message. If any rules
// are violated, an error is returned.
func (m *AssertGatewayRightsResponse) ValidateFields(paths ...string) error {
	if m == nil {
		return nil
	}

	if len(paths) == 0 {
		paths = AssertGatewayRightsResponseFieldPathsNested
	}
	...
	for name, subs := range _processPaths(append(paths[:0:0], paths...)) {
		...
		case "gateway_ids":
			...
			if len(m.GetGatewayIds()) > 20 {
				return AssertGatewayRightsResponseValidationError{
					field:  "gateway_ids",
		...
		case "has_required_rights":
			// no validation rules for HasRequiredRights
		default:
			return AssertGatewayRightsResponseValidationError{
				field:  name,
				reason: "invalid field path",
...
// AssertGatewayRightsResponseValidationError is the validation error returned
// by AssertGatewayRightsResponse.ValidateFields if the designated constraints
// aren't met.
type AssertGatewayRightsResponseValidationError struct {
	field  string
	reason string
	cause  error
	key    bool
...
// Field function returns field value.
func (e AssertGatewayRightsResponseValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e AssertGatewayRightsResponseValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e AssertGatewayRightsResponseValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e AssertGatewayRightsResponseValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e AssertGatewayRightsResponseValidationError) ErrorName() string {
	return "AssertGatewayRightsResponseValidationError"
}

// Error satisfies the builtin error interface
func (e AssertGatewayRightsResponseValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
	...
	return fmt.Sprintf(
		"invalid %sAssertGatewayRightsResponse.%s: %s%s",
		key,
		e.field,
		e.reason,
		cause)
...
var _ error = AssertGatewayRightsResponseValidationError{}
...
var _ interface {
	...
	Cause() error
	ErrorName() string
} = AssertGatewayRightsResponseValidationError{}
