...
func recordSchemaVersion(cl *ttnredis.Client) error {
	logger.WithField("version", networkserver.SchemaVersion).Info("Setting schema version")
	return cl.Set(ctx, "schema-version", networkserver.SchemaVersion, 0).Err()
}
...
func checkLatestSchemaVersion(cl *ttnredis.Client) (bool, error) {
	...
	return schemaVersion < networkserver.SchemaVersion, nil
}
