...
The stack binary, or the component binaries, can be configured by passing parameters.

## Default config

The binaries can be used for testing purposes without any additional configuration. To test and use the components to the fullest, you can use the `--help` flag to show available parameters, and refer to execution logs for recommendations.
...
You can specify a YAML configuration file with the `-c` flag. `~/.ttn-lw-<binary type>.yml` is used by default, with the binary type being `stack`, `gateway-server`, `network-server` or others.
