...
| `received_at` | [`google.protobuf.Timestamp`](#google.protobuf.Timestamp) |  | Server time when the Network Server received the message. This field is not validated in the API since ApplicationUplink is used also in AS packages. Validate this separately in the RPCs that processes messages from the NS. |
