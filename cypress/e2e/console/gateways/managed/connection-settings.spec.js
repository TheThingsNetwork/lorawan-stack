...
describe('Managed Gateway connection settings', () => {
  ...
  describe('With WiFi and Ethernet', () => {
    ...
    beforeEach(() => {
      ...
      cy.intercept('GET', `/api/v3/gcs/gateways/managed/${gatewayId}*`, {
        ...
        body: {
          ...
          capabilities: {
            wifi_2_4_ghz: true,
            scan_wifi_access_points: true,
            ethernet: true,
            cellular: true,
            lora_8ch_concentrator: true,
    ...
    it('succeeds to set WiFi connection with already created profile', () => {
      ...
      cy.findByText(
        'The gateway WiFi is currently attempting to connect using this profile',
      ).should('be.visible')
      cy.findByTestId('error-notification').should('not.exist')
    ...
    it('succeeds to validate new WiFi profile fields', () => {
      ...
      cy.get('#wifi_profile\\.network_interface_addresses\\.gateway-field-error').should(
        'be.visible',
      )
    })
  ...
  describe('Without WiFi and Ethernet', () => {
    beforeEach(() => {
      cy.dropAndSeedDatabase()
      cy.createUser(user)
      cy.createGateway(gateway, userId)
    ...
    it('succeeds to display UI elements in place', () => {
      cy.findByTestId('notification')
        .should('be.visible')
        .and('contain', 'This gateway does not have any connection settings to configure.')
      cy.findByText('WiFi connection', { selector: 'h3' }).should('not.exist')
      cy.findByText('Ethernet connection', { selector: 'h3' }).should('not.exist')
