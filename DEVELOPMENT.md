...
The Things Network's development environment heavily relies on [`make`](https://www.gnu.org/software/make/). Under the hood, `make` calls other tools such as `git`, `go`, `yarn` etc. Recent versions are supported; Node v10.x and Go v1.12. Let's first make sure you have `go`, `node` and `yarn`:
...
curl -sSL https://dl.google.com/go/go1.12.linux-amd64.tar.gz | sudo tar -xz -C /usr/local
